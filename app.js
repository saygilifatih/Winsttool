...
const shopIds = ['ads','rent','shopOther','shopPurchase','shopFulfilment'];
const defaults = { fixedCommission:'1', commissionRate:'15', bolFulfilment:'', shipping:'', bolOther:'', ads:'', rent:'', shopOther:'', shopPurchase:'', shopFulfilment:'' };
let settings = { ...defaults }, mode = 'bol', period = 1;
try { settings = { ...defaults, ...JSON.parse(localStorage.getItem('winsttool-settings')) }; } catch (_) {}
try { const state = JSON.parse(localStorage.getItem('winsttool-state')); mode = state?.mode || mode; period = state?.period || period; ['sale','purchase','revenue','orders'].forEach(id => $(id).value = state?.[id] || ''); } catch (_) {}
[...bolIds, ...shopIds].forEach(id => $(id).value = settings[id]);
function saveState() { localStorage.setItem('winsttool-state', JSON.stringify({ mode, period, sale:$('sale').value, purchase:$('purchase').value, revenue:$('revenue').value, orders:$('orders').value })); }
function renderMode() {
...
function calculate() {
  ...
  else { net = read('revenue') / 1.21; middle = (read('ads') + read('rent') + read('shopOther')) * period; costs = middle + read('orders') * (read('shopPurchase') + read('shopFulfilment')); }
...
['sale','purchase','revenue','orders', ...bolIds, ...shopIds].forEach(id => $(id).addEventListener('input', calculate));
